...
fun KSPropertyDeclaration.getWrapperName(type: ReturnType): String {
    return when (type) {
        is ReturnType.Value -> getSimpleMemberName()
        is ReturnType.Flow, is ReturnType.StateFlow -> getTransformedMemberName()
        else -> throw IllegalStateException("Unknown return type ${type::class}")
    }
}

fun KSFunctionDeclaration.getWrapperName(type: FunctionType): String {
    return when (type) {
        ...
        is FunctionType.Suspending -> getTransformedMemberName()
        else -> throw IllegalStateException("Unknown function type ${type::class}")
    }
}

private fun KSDeclaration.getSimpleMemberName(): String =
    Options.Generator.getSimpleMemberName(simpleName.asString())

private fun KSDeclaration.getTransformedMemberName(): String =
